...
#include "Bluetooth.h"//Parametros de referencias --> bluetooth
...
void setup()
{
  Serial.begin(115200);//Se inicializa puerto serial

  BT.begin("ESP32_robot_e4");//se inicializa la comunicación Bluetooth
  
  Serial.println("El dispositivo Bluetooth esta activado");
...
void loop()
{
  ...
  //Se revisa referencia de velocidad (en frecuencia khz) -->
  if (BT.available())//si hay conexión bluetooth se escuchan datos de entrada
  {
    data = BT.read();//se recibe dato
    //Serial.println(data);
    //Serial.print("\t");
    ...
    switch(data)
    {
      case '0': porcentaje_v = 0; break;//0%de velocidad
      case '1': porcentaje_v = 10; break;//10%de velocidad
      case '2': porcentaje_v = 20; break;//20%de velocidad
      case '3': porcentaje_v = 30; break;//30%de velocidad
      case '4': porcentaje_v = 40; break;//40%de velocidad
      case '5': porcentaje_v = 50; break;//50%de velocidad
      case '6': porcentaje_v = 60; break;//60%de velocidad
      case '7': porcentaje_v = 70; break;//70%de velocidad
      case '8': porcentaje_v = 80; break;//80%de velocidad
      case '9': porcentaje_v = 90; break;//90%de velocidad 
      case 'q': porcentaje_v = 100; break;//100% de velocidad
      default: //se le da instrucción al robot
        //Serial.print(porcentaje_v);
        switch (data)
        {
          case 'S'://Stop el robot no avanza --> no se presiona ningún botón 
            vd = 0;//velocidad traslacional deseada
            omegad = 0;//velocidad angular deseada
            break;
    
          //Casos de rotar 
          case 'R'://El robot gira hacia la derecha --> no avanza solo rota en sentido horario (positivo)
            vd = 0;//velocidad traslacional deseada
            omegad = (porcentaje_v * omega_max)/100;;//velocidad angular deseada
            break;

           case 'L'://El robot gira hacia la izquierda --> no avanza solo rota en sentido antihorario (negativo)
            vd = 0;//velocidad traslacional deseada
            omegad = -(porcentaje_v * omega_max)/100;;//velocidad angular deseada
            break;

          //Referencias positivas
          case 'F'://Forward el robot avanza conforme al %de velocidad especificado, solo avanza no rota
            vd = (porcentaje_v * v_max)/100;//velocidad traslacional deseada
            omegad = 0;//velocidad angular deseada
            break;
            
          case 'I'://E --> Forward right, el robot avanza y gira hacia la derecha --> sentido horario (positivo)
            vd = ((porcentaje_v * v_max)/100)* factor_limit;//velocidad traslacional deseada, se le baja 25% de velocidad
            omegad = ((porcentaje_v * omega_max)/100)* factor_limit;//velocidad angular deseada
            break;
       
          case 'G'://E --> Forward left, el robot avanza y gira hacia la izquierda --> sentido antihorario (negativo)
            vd = ((porcentaje_v * v_max)/100)* factor_limit;//velocidad traslacional deseada, se le baja 25% de velocidad
            omegad = -((porcentaje_v * omega_max)/100)* factor_limit;//velocidad angular deseada
            break;
                
          //Referencias negativas
          case 'B'://Backward el robot avanza en reversa conforme al %de velocidad especificado, solo avanza no rota
            vd = -(porcentaje_v * v_max)/100;//velocidad traslacional deseada
            omegad = 0;//velocidad angular deseada
            break;
         
          case 'J'://Back right --> el robot gira hacia la derecha en reversa --> sentido horario (positivo)
            vd = -((porcentaje_v * v_max)/100)* factor_limit;//velocidad traslacional deseada, se le baja 25% de velocidad
            omegad = ((porcentaje_v * omega_max)/100)* factor_limit;//velocidad angular deseada
            break;
            
          case 'H'://Back left --> el robot gira hacia la izquierda en reversa --> sentido antihorario (negativo)
            vd = -((porcentaje_v * v_max)/100)* factor_limit;//velocidad traslacional deseada, se le baja 25% de velocidad
            omegad = -((porcentaje_v * omega_max)/100)* factor_limit;//velocidad angular deseada
            break;
            
          default:
            Serial.println("no entendí"); 
            break;
          }
          break;
      }
